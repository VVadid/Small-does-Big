...
export(String) var timeline_to_play := ""  # Timeline for main dialogue

# New cross dialogue system
export(bool) var has_cross_dialogue := false
export(String) var cross_dialogue_timeline := ""
export(int) var cross_required_stars := 0
...
func _on_DetectionArea_body_entered(body):
	if (body.is_in_group("player") 
		...
		# Check for cross dialogue first (if enabled and main dialogue already played)
		if has_cross_dialogue and global.played_dialogues.get(dialogue_id + "_main", false):
			if global.stars >= cross_required_stars:
				prepare_for_cross_dialogue(body)
			else:
				play_no_star_dialogue(body)
		# Normal flow - only check played_dialogues for main dialogue
		elif !global.played_dialogues.get(dialogue_id, false):
			if global.stars >= required_stars:
				prepare_for_dialogue(body)
			else:
				play_no_star_dialogue(body)

func prepare_for_cross_dialogue(player):
	is_chatting = true
	...
	start_cross_dialogue(player)

func start_cross_dialogue(player):
	var dialog_control = $DialogicControl
	dialog_node = dialog_control.start_specific_dialogue(cross_dialogue_timeline)
	dialog_node.connect("timeline_end", self, "_on_cross_dialogue_end", [player], CONNECT_ONESHOT)
...
func _on_cross_dialogue_end(_timeline_name, player):
	...
	# DON'T queue_free - keep NPC for future interactions
...
func start_dialogue(player):
	...
	# Mark main dialogue as played for cross dialogue system
	global.played_dialogues[dialogue_id + "_main"] = true
	
	var dialog_control = $DialogicControl
	# Use timeline_to_play for main dialogue
	if timeline_to_play != "":
...
func _on_dialogue_end(_timeline_name, player):
	...
	# Only queue_free if cross dialogue is NOT enabled
	if !has_cross_dialogue:
		queue_free()
